...
class OnMessage (commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if last_message is None or last_message == 0 or current_time - last_message >= 60:
            ...
            # Checks if the user has boosted the server
            ...
            # Saves the new data to the user
        ...
        if message.channel == suggest_channel and message.content.startswith('.s '):
            if re.search(url_pattern, message.content):
                ...
                tinky = self.bot.get_emoji(1295755914862923837)
                emery = self.bot.get_emoji(1295755990544941127)
                if tinky and emery:
                ...
                await message.delete()
        elif message.channel == suggest_channel and not message.content.startswith('.s '):
            print(f"El mensaje de {message.author} no comienza con '.s ' en el canal {message.channel.name}")
            try:
                await message.delete()
            except discord.errors.NotFound:
                print(f"El mensaje con ID {message.id} ya no existe o fue eliminado anteriormente.")
            except discord.errors.Forbidden:
                print("No tengo permisos para eliminar este mensaje.")
            user = message.author  # Obtén el autor del mensaje
            try:
                # Envía un mensaje directo al usuario
                ...
                # Si el bot no puede enviar DM (por ejemplo, si el usuario tiene los DMs cerrados)
                print(f"No se pudo enviar DM a {user.name}.")
            if message.channel == allowed_channel:
