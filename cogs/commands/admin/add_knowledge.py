...
class AddKnowledge(commands.Cog):
    ...
    async def add_knowledge(self, interaction:discord.Interaction, usuario: discord.Member, cantidad: int):
        ...
        try:
            if cantidad <= 1:
                await usuario.send(f"Se te ha añadido {cantidad} <:knowledge:1338469359906979874>")
            else:
                await usuario.send(f"Se te han añadido {cantidad} <:knowledge:1338469359906979874>")
