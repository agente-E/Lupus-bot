...
class RmNotes(commands.Cog):
    ...
    @app_commands.describe(usuario="Usuario al que quitar Notes")
    @app_commands.describe(cantidad="Número de Notes a quitar")
    async def add_notes(self, interaction:discord.Interaction, usuario: discord.Member, cantidad: int):
        ...
        if not await self.database.decrease_notes(usuario.id, cantidad):
            await interaction.followup.send(f"Error añadiendo el item (mira la consola).")
            return

        await interaction.followup.send(f"{cantidad}<:note:1338471019702259763> extraidos al usuario {usuario.name}", ephemeral=True)
        try:
            message = None
            if cantidad <= 1:
                message = f"Se te ha eliminado {cantidad}<:note:1338471019702259763>"
            else:
                message = f"Se te han eliminado {cantidad}<:note:1338471019702259763>"
