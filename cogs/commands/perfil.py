...
async def perfil(interaction: discord.Interaction, user: discord.User = None):
    ...
    embed.add_field(name="Experiencia", value=str(user_data['experiencia']), inline=True)
    embed.add_field(name="Nivel", value=str(user_data['nivel']), inline=True)
    embed.add_field(name="Pity Counter", value=str(user_data['pity_counter']), inline=True)
